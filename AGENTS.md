...
- The Accessibility Nutrition Label (`A11Y` const in `store-ios.mjs`) declares VoiceOver, Larger Text, Dark Interface, and Differentiate Without Color Alone for iPhone + iPad. Apple's bar is "all common tasks completable with the feature" — re-verify with a VoiceOver pass before adding a claim.
