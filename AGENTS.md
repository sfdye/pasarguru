...
- Every icon raster in `assets/` is **generated**: `npm run icons` derives them from `brand/` (Pillow + librsvg, hand-run). Edit the master in `brand/`, never the output. The 96px notification glyph is the exception — a hand-drawn SVG. `onemap-logo.png`, `app-store-badge.svg` and `google-play-badge.png` are third-party compliance/licence artwork (OneMap, Apple, Google), not generated. Both store badges are deliberately tweaked from the official downloads (App Store SVG intrinsic size doubled for GitHub's shrink-only image sizing; Play PNG margin cropped) — don't restore them.

## Store metadata (asc for iOS, fastlane for Android)

EAS keeps builds, binary submission, and OTA. iOS App Store state (metadata text, screenshots, categories, copyright, review notes) is pushed by `scripts/store-ios.mjs` through the `asc` CLI. Android stays on fastlane `supply`; `asc` deliberately has no Android support.
...
- iOS listing lives in `store/ios/`: `metadata/` is asc's JSON layout (`app-info/` is app-level, `version/<version>/` is per store version — when bumping `version` by hand, `git mv` the existing `version/` dir to match; the dir is singular, unlike asc's docs) — files are kept byte-identical to `asc metadata pull` output (compact, no trailing newline) so pulls never diff, which is also why descriptions carry `\n` escapes: JSON strings cannot hold literal newlines; `screenshots/{en-US,zh-Hans}/{iphone,ipad}/`. Android in `fastlane/metadata/android/{en-US,zh-CN}/` — note the different locale codes for Simplified Chinese.
